                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            SellerBidsList.Add(new BuyerBid
                            {
                                ...
                                BidFromBuyer = Convert.ToDecimal(reader["OfferPriceBuyer"]),
                                Comments = reader["Comments"].ToString(),
                                CompanyName = reader["Razon_Social"].ToString(),
                                UserFullName = reader["FullName"].ToString(),
                                SellerFullName = reader["UserSeller"].ToString(),
                                SellerCompany = reader["CompanySeller"].ToString(),
                                SellerOfferPrice = Convert.ToDecimal(reader["PriceSeller"]),
                                IdStatus = Convert.ToInt32(reader["IdStatus"]),
                                BidStatus = reader["BidStatus"].ToString()
                            });
                        }
                    }
                }
        ...
        public async Task UploadBid(Bids bid)
        {
            try
            {
                await using (SqlConnection cn = new(sqlString.GetSqlString()))
                {
                    ...
                    cmd.Parameters.AddWithValue("OfferPriceBuyer", bid.OfferPriceBuyer);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.ExecuteNonQuery();
                }
            }
